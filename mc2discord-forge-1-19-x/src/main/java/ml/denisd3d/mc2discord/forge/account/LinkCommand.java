...
public class LinkCommand {
        ...
        LiteralArgumentBuilder<CommandSourceStack> command = Commands.literal(strings[strings.length - 1]).executes(context -> {
            ...
            if (Mc2Discord.INSTANCE.m2dAccount != null) { // Should always be the case at this point
                ...
                if (result != null) {
                    ...
                    context.getSource().sendFailure(Component.literal(Mc2Discord.INSTANCE.config.account.messages.link_error_already));
                }
            }
            return 1;
        });
    ...
    public static Component getCopiableTextComponent(String result) {
        Matcher matcher = pattern.matcher(result);

        MutableComponent textComponent = Component.literal("");
        int index = 0;
        while (matcher.find()) {
            textComponent.append(Component.literal(result.substring(index, matcher.start(1))));
            textComponent.append(Component.literal(matcher.group(2))
                    .withStyle(style -> style.withClickEvent(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, matcher.group(2)))
                            .withColor(TextColor.fromLegacyFormat(ChatFormatting.BLUE))
                            .withUnderlined(true)));
            index = matcher.end();
        }
        textComponent.append(Component.literal(result.substring(index)));
