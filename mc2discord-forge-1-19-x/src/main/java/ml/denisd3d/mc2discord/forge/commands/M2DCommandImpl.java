...
public class M2DCommandImpl {
    ...
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
        dispatcher.register(Commands.literal("mc2discord")
                ...
                .then(Commands.literal("status").executes(context -> {
                    M2DCommands.getStatus().forEach(s -> context.getSource().sendSuccess(Component.literal(s), false));
                    return 1;
                }))
                .then(Commands.literal("restart").executes(context -> {
                    M2DCommands.restart().forEach(s -> context.getSource().sendSuccess(Component.literal(s), false));
                    return 1;
                }))
                .then(Commands.literal("upload").executes(context -> {
                    String[] result = M2DCommands.upload();
                    context.getSource()
                            .sendSuccess(Component.literal(result[0])
                                    .append(Component.literal(result[1])
                                            .withStyle(style -> style.withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, result[1]))
                ...
                .then(Commands.literal("invite").executes(context -> {
                    ...
                    context.getSource()
                            .sendSuccess(Component.literal(result)
                                    .withStyle(style -> style.withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, result))
                                            .withColor(TextColor.fromLegacyFormat(ChatFormatting.BLUE))
                                            .withUnderlined(true)), false);
                    return 1;
                }))
    ...
    private static int reloadLinkedPlayers(CommandSourceStack source) {
        if (Mc2Discord.INSTANCE.m2dAccount == null || !Mc2Discord.INSTANCE.config.features.account_linking) {
            source.sendFailure(Component.literal("Account linking features isn't enabled."));
            return 0;
        }
        Mc2Discord.INSTANCE.m2dAccount.iAccount.loadDiscordIds();
        source.sendSuccess(Component.literal(LangManager.translate("commands.linked.reload")), true);
        return 1;
    }
    ...
    private static int removeLinkedPlayers(CommandSourceStack source, Collection<GameProfile> targets) throws CommandSyntaxException {
        ...
        for (GameProfile gameprofile : targets) {
            if (iAccount.contains(gameprofile)) {
                ...
                source.sendSuccess(Component.literal(LangManager.translate("commands.linked.unlinked", ComponentUtils.getDisplayName(gameprofile)
                        .getString())), true);
                ++i;
            }
        }
    ...
    private static int addLinkedPlayers(CommandSourceStack source, String target, int discord_id) throws CommandSyntaxException {
        ...
        if (gameprofile == null || gameprofile.getId() == null) {
            source.sendFailure(Component.literal("Can't link this player by name. Please use a valid player uuid"));
            return 0;
        }

        if (!iAccount.contains(gameprofile)) {
            iAccount.add(gameprofile.getId(), discord_id);
            source.sendSuccess(Component.literal(LangManager.translate("commands.linked.linked", ComponentUtils.getDisplayName(gameprofile)
                    .getString())), true);
            return 1;
        } else {
    ...
    private static int listLinkedPlayers(CommandSourceStack source) {
        ...
        if (entries.size() == 0) {
            source.sendSuccess(Component.literal(LangManager.translate("commands.linked.empty")), false);
        } else {
            source.sendSuccess(Component.literal(LangManager.translate("commands.linked.list", entries.size(), entries.stream()
                    .map(StoredUserEntry::getUser)
                    .filter(Objects::nonNull)
                    .map(gameProfile -> {
                        if (gameProfile.getName() != null) {
                            ...
                            Optional<GameProfile> optionalGameProfile = ServerLifecycleHooks.getCurrentServer()
                                    .getProfileCache()
    ...
    private static int reloadHiddenPlayers(CommandSourceStack source) {
        ...
        source.sendSuccess(Component.literal(LangManager.translate("commands.hidden.reload")), true);
        return 1;
    }
    ...
    private static int addHiddenPlayers(CommandSourceStack source, Collection<GameProfile> players) throws CommandSyntaxException {
        ...
        for (GameProfile gameprofile : players) {
            if (!hiddenPlayerList.isHidden(gameprofile)) {
                ...
                source.sendSuccess(Component.literal(LangManager.translate("commands.hidden.hidden", ComponentUtils.getDisplayName(gameprofile)
                        .getString())), true);
                ++i;
            }
        }
    ...
    private static int removeHiddenPlayers(CommandSourceStack source, Collection<GameProfile> players) throws CommandSyntaxException {
        ...
        for (GameProfile gameprofile : players) {
            if (hiddenPlayerList.isHidden(gameprofile)) {
                ...
                source.sendSuccess(Component.literal(LangManager.translate("commands.hidden.visible", ComponentUtils.getDisplayName(gameprofile)
                        .getString())), true);
                ++i;
            }
        }
    ...
    private static int listHiddenPlayers(CommandSourceStack source) {
        ...
        if (hiddenPlayers.length == 0) {
            source.sendSuccess(Component.literal(LangManager.translate("commands.hidden.empty")), false);
        } else {
            source.sendSuccess(Component.literal(LangManager.translate("commands.hidden.list", hiddenPlayers.length, String.join(", ", hiddenPlayers))), false);
        }
