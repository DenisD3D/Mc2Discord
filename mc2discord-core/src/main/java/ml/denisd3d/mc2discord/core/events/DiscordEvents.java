    ...
    public static void onDiscordMessageReceived(MessageCreateEvent messageCreateEvent) {
        ...
        if (!messageCreateEvent.getMessage().getAuthor().isPresent()) // It's a webhook
        {
            messageCreateEvent.getMessage()
                    ...
                    .map(s -> s.orElse(""))
                    .subscribe(s -> {
                        if (Mc2Discord.INSTANCE.config.misc.relay_bot_messages && !s.equals("Mc2Dis Webhook - " + Mc2Discord.INSTANCE.botName + "#" + Mc2Discord.INSTANCE.botDiscriminator) && Mc2Discord.INSTANCE.config.channels.channels.stream()
