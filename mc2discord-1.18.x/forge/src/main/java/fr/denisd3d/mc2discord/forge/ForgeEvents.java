...
public class ForgeEvents {
    ...
    public static void onServerChat(ServerChatEvent event) {
        ...
        MinecraftEvents.onMinecraftChatMessageEvent(event.getMessage(), new PlayerEntity(event.getPlayer().getGameProfile().getName(), event.getPlayer()
                .getDisplayName()
    ...
    public static void onPlayerConnectEvent(PlayerEvent.PlayerLoggedInEvent event) {
        MinecraftEvents.onPlayerConnectEvent(new PlayerEntity(event.getPlayer().getGameProfile().getName(), event.getEntity().getDisplayName().getString(), event.getPlayer().getGameProfile().getId()));
    }

    @SubscribeEvent
    public static void onPlayerDisconnectEvent(PlayerEvent.PlayerLoggedOutEvent event) {
        MinecraftEvents.onPlayerDisconnectEvent(new PlayerEntity(event.getPlayer().getGameProfile().getName(), event.getEntity().getDisplayName().getString(), event.getPlayer().getGameProfile().getId()));
    }
    ...
    public static void onAdvancementEvent(AdvancementEvent event) {
        if (event.getAdvancement().getDisplay() != null && event.getAdvancement().getDisplay().shouldAnnounceChat()) {
            MinecraftEvents.onAdvancementEvent(new PlayerEntity(event.getPlayer().getGameProfile().getName(), event.getEntity().getDisplayName().getString(), event.getPlayer()
                    .getGameProfile()
