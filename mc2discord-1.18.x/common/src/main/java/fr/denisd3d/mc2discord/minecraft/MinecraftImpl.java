...
public class MinecraftImpl implements IMinecraft {
    ...
    public static MutableComponent convertToComponent(String content, Map<String, MutableComponent> replacements) {
        ...
        while (matcher.find()) {
            ...
            component.append(new TextComponent(content.substring(lastAppendPosition, matcher.start())).withStyle(baseStyle));
            ...
            if (variable_name != null) {
                ...
                component.append(new TextComponent(url).withStyle(baseStyle).withStyle(style -> style.withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, url))));
            }
        }
        component.append(new TextComponent(content.substring(lastAppendPosition)).withStyle(baseStyle));
        return component;
    }
    ...
    public void sendMessage(String content, HashMap<String, String> attachments, String referencedContent, String senderUsername) {
        ...
        MutableComponent attachementsComponent = new TextComponent(" ");
        for (Map.Entry<String, String> entry : attachments.entrySet()) {
            attachementsComponent.append(new TextComponent("[" + entry.getKey() + "]").withStyle(style -> style.withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_URL, entry.getValue())).withColor(ChatFormatting.BLUE).withUnderlined(true)));
        }
        replacements.put("attachments", attachementsComponent);

        if (referencedContent != null) {
            replacements.put("reply", new TextComponent(referencedContent));
        }
        ...
        Mc2DiscordMinecraft.server.getPlayerList().broadcastMessage(component, ChatType.CHAT, Util.NIL_UUID);
    }
    ...
    public void executeCommand(String command, int permissionLevel, Snowflake channelId) {
        ...
        Mc2DiscordMinecraft.server.getCommands().performCommand(Mc2DiscordMinecraft.commandSource.withPermission(permissionLevel), command);
    }
    ...
    public GlobalEntity getServerData() {
        ...
        return new GlobalEntity(onlinePlayerCount, server.getMaxPlayers(), Optional.ofNullable(server.playerDataStorage.playerDir.listFiles((dir, name) -> name.toLowerCase().endsWith(".dat"))).map(files -> files.length).orElse(0), server.getMotd(), server.getServerVersion(), server.getLocalIp(), String.valueOf(server.getPort()), tps, mspt);
    }
