                    ...
                    if (Mc2Discord.INSTANCE.m2dAccount != null) { // Should always be the case at this point
                        if (Mc2Discord.INSTANCE.m2dAccount.iAccount.remove(player.getGameProfile().getId())) {
                            if (Mc2Discord.INSTANCE.config.account.force_link) {
                                ...
                                context.getSource()
                                        .sendSuccess(new TextComponent(Mc2Discord.INSTANCE.config.account.messages.unlink_successful), false);
                            }
                        } else {
                            context.getSource()
                                    .sendFailure(new TextComponent(Mc2Discord.INSTANCE.config.account.messages.unlink_error));
                        }
                    }
                    return 1;
                });
